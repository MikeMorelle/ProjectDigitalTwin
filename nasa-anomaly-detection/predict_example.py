...
Alert function is imported from alert_system.py
"""
...
# Import the alert function from the separate file
...
print(f"Alert system: Score-based (OK / ALERT) - imported from alert_system.py")
print()

# Get the data file from user input or command line
if len(sys.argv) > 1:
...
# Detect which dataset we are using (FD001 - FD004)
dataset_num = int(data_file.split('FD00')[1][0])
print(f"Detected: FD00{dataset_num}")

# Preprocessing: clustering and normalization
if dataset_num in [2, 4]:
    # FD002 and FD004: 6 operating conditions → cluster first
    ...
    # FD001 and FD003: single condition → scale directly
...
# Compute rolling mean and std for each sensor
...
# Determine alert levels using the imported function (NO RUL needed!)
...
# Show which engines triggered ALERT
if alerted_engines > 0:
    ...
    for eng in sorted(data[data['alert_level'] == 'ALERT']['unit_id'].unique()):
        ...
        print(f"  Engine #{eng}: First alert at cycle {first_alert:.0f} ({len(eng_data)} alert cycles)")
